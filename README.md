...
В лабе на 4 использовал структуры для облегчения читаемости. Сложнее алгоритмы - больше объясняющих комментариев  
...
# программы
mise 3
mise 4 -- [OPTIONS] COMMAND INPUT_FILE
```
...
# запуск
python3 -m lab3
python3 -m lab4 -- [OPTIONS] COMMAND INPUT_FILE
```

## На 3

`mise 3` или `python3 -m lab3`
...
При наличии в файле одинаковых рёбер используюется последняя, в остальном порядок не важен
...
`mise 4 -- [OPTIONS] COMMAND INPUT_FILE` или `python3 -m lab3 -- [OPTIONS] COMMAND INPUT_FILE`

### Доступные команды:

- `max_flow` - Нахождение максимального потока в сети
- `shortest_path` - Нахождение короткого пути алгоритмом A* с Евклидовой и Манхэттенской эвристиками
  - Аргументы: `-a start_row start_column` `-b goal_row goal_column`
- `matching` - Проверка двудольности графа, нахождение максимального паросочетания
